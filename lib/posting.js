...
import { SEED_ACCOUNTS, findBySystemKey, findById } from "@/lib/accounts";
...
export function ensureAccounts(db) {
  ...
  for (const seed of SEED_ACCOUNTS) {
    const exists = seed.systemKey
      ? db.accounts.some((a) => a.systemKey === seed.systemKey)
      : db.accounts.some((a) => a.number === seed.number);
    if (exists) continue;
    db.accounts.push({
      id: uid(),
      number: seed.number,
