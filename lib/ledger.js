...
//   Παραλαβή Παραγγελίας Αγοράς (αγορά αποθέματος, πληρωμένη τη στιγμή της παραλαβής):
//     Χ Απόθεμα ανά είδος (= qty × unitCost)                       Π Ταμείο/Τράπεζα (= σύνολο)
//   Παραλαβή επί πιστώσει (θα πληρωθεί αργότερα στον προμηθευτή):
//     Χ Απόθεμα ανά είδος (= qty × unitCost)                       Π Προμηθευτές (= σύνολο)
//   Εξόφληση προμηθευτή αργότερα:
//     Χ Προμηθευτές (= amount)                                     Π Ταμείο/Τράπεζα (= amount)
//
...
export function buildLedgerEntries(db, from = "0000-01-01", to = "9999-12-31") {
  ...
  for (const po of db.purchases || []) {
    ...
    // "credit" = επί πιστώσει (θα πληρωθεί αργότερα) — πιστώνει Προμηθευτές, όχι Ταμείο/Τράπεζα·
    // η εξόφληση καταχωρείται ξεχωριστά παρακάτω (βλ. db.supplierPayments) όποτε πράγματι γίνει.
    lines.push({ account: po.paymentMethod === "credit" ? "payable" : cashOrBank(po.paymentMethod), debit: 0, credit: totalCost });
    entries.push({
  ...
  // Εξόφληση προμηθευτή (πληρωμή μιας αγοράς επί πιστώσει, βλ. πάνω) — μεταφέρει το ποσό από
  // Προμηθευτές σε Ταμείο/Τράπεζα, όποτε πραγματικά πληρωθεί.
  for (const sp of db.supplierPayments || []) {
    if (sp.date < from || sp.date > to) continue;
    const amount = round2(sp.amount);
    if (!amount) continue;
    const account = cashOrBank(sp.method);
    entries.push({
      id: `sp-${sp.id}`, date: sp.date, createdAt: sp.createdAt, ref: "",
      descKey: "journal.descSupplierPayment", descParams: { supplier: sp.supplierName || "" },
      lines: [{ account: "payable", debit: amount, credit: 0 }, { account, debit: 0, credit: amount }],
    });
  }
