...
import { extractColumnInvoice } from "./pdfInvoiceColumns.js";
...
export async function extractInvoiceItems(buffer) {
  ...
  try {
    ...
    // Τιμολόγια με στήλες QTY | PRICE | DIS% | NET | AMOUNT (π.χ. Melanico/Eurosoft), ιδίως σκαναρισμένα με κινητό:
    // αναγνώστης ανά στήλη, με έλεγχο των αθροισμάτων με τα σύνολα του ίδιου του τιμολογίου (βλ. pdfInvoiceColumns.js).
    try {
      const cols = await extractColumnInvoice(buffer);
      if (cols && cols.items.length > 0) return { items: cols.items, source: "columns", check: cols.check };
    } catch (e) {
      console.error("Αποτυχία ανάγνωσης τιμολογίου ανά στήλη, fallback σε γενικό αναγνώστη:", e);
    }
