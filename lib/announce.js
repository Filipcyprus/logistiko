...
export function discountLines(customer, lang) {
  ...
  for (const r of customer.discountRules || []) {
    const key = r.type === "brand" ? "announce.discBrand" : r.type === "subcategory" ? "announce.discSubcategory" : "announce.discCategory";
    lines.push(t(key, { name: r.value, percent: Number(r.percent) }));
  }
