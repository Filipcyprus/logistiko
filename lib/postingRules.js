...
export function entryForExpense(db, expense) {
  ...
  const lines = [];
  // Ο χρήστης μπορεί να διαλέξει συγκεκριμένο λογαριασμό εξόδου· αλλιώς πάει στα γενικά έξοδα.
  if (net) lines.push(expense.accountId ? { accountId: expense.accountId, debit: net, credit: 0 } : { systemKey: "expensesNet", debit: net, credit: 0 });
  if (vat) lines.push({ systemKey: "vatInput", debit: vat, credit: 0 });
  lines.push({ systemKey: cashOrBank(expense.paymentMethod), debit: 0, credit: amount });
