...
import { purchaseVat, lineNet, effectiveQty } from "@/lib/purchaseMath";
...
//   Αγορά (πληρωμένη):      Χ Απόθεμα ανά είδος + ΦΠΑ Εισροών   Π Ταμείο/Τράπεζα (καθαρό + ΦΠΑ)
//   Αγορά επί πιστώσει:     Χ Απόθεμα ανά είδος + ΦΠΑ Εισροών   Π Προμηθευτές (καθαρό + ΦΠΑ)
...
export function entryForPurchaseReceipt(db, po) {
  ...
  const items = (po.items || []).filter((it) => Number(it.unitCost) > 0 && effectiveQty(it) > 0);
  ...
  // ΦΠΑ που πληρώθηκε στον προμηθευτή: δεν είναι κόστος αποθέματος — είναι ΦΠΑ εισροών (2110) που
  // συμψηφίζεται στη δήλωση ΦΠΑ, ακριβώς όπως στα έξοδα. Παλιές γραμμές χωρίς vatRate = 0.
  const vat = purchaseVat({ items });
  if (vat) lines.push({ systemKey: "vatInput", debit: vat, credit: 0 });

  // Ο προμηθευτής πληρώνεται (ή του οφείλουμε) το ΣΥΝΟΛΟ: καθαρό + ΦΠΑ.
  lines.push({
    systemKey: po.paymentMethod === "credit" ? "payable" : cashOrBank(po.paymentMethod),
    debit: 0,
    credit: round2(totalCost + vat),
  });
