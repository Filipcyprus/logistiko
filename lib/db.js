...
const DEFAULT_DATA = {
  settings: {
    ...
    vatRate: 19, // Κύπρος
    // Η επιχείρηση δεν είναι (ακόμα) εγγεγραμμένη στο ΦΠΑ: το B2B link δεν χρεώνει ΦΠΑ και το εξηγεί στον πελάτη.
    ...
    expensePrefix: "EXP-",
    journalPrefix: "JE-",
    // Κλείσιμο περιόδου: μέχρι κι αυτή την ημερομηνία τα βιβλία είναι κλειδωμένα — καμία νέα
    // εγγραφή, καμία διαγραφή. Κενό = ανοιχτά βιβλία. Βλ. lib/posting.js.
    lockedThrough: "",
    series: "A",
  ...
  purchases: [],
  payments: [],
  // Πληρωμές ΠΡΟΣ προμηθευτές (εξόφληση αγορών επί πιστώσει) — καθρέφτης του "payments" (εισπράξεις
  // ΑΠΟ πελάτες) αλλά για την αντίθετη κατεύθυνση. Βλ. app/api/supplier-payments/route.js.
  supplierPayments: [], // { id, supplierId, purchaseId, date, amount, method, notes, createdAt }
  expenses: [],
  stockMovements: [],
  activities: [],
  // Λογιστικό Σχέδιο — αρχικοποιείται από το lib/accounts.js την πρώτη φορά (βλ. ensureAccounts).
  accounts: [], // { id, number, name, nameKey, systemKey, type, isSystem, active, createdAt }
  // Γενικό Καθολικό — ΚΑΘΕ λογιστική κίνηση, αμετάβλητη μόλις καταχωριστεί. Όλες οι λογιστικές
  // καταστάσεις (Ισοζύγιο, Καθολικό, Αποτελέσματα, Ισολογισμός) προκύπτουν ΜΟΝΟ από εδώ.
  announcements: [], // { id, createdAt, subject, body, kind, channels{email,portal}, audience[], results[], sent, failed, skipped }
  journalEntries: [], // { id, number, date, memo, source, lines[], createdAt, reversalOf, reversedBy }
  // Κλεισμένες (οριστικές) Ζ αναφορές — μόλις κλείσει μία περίοδος, ο αριθμός και τα ποσά της
  // παγώνουν για πάντα (φορολογική απαίτηση: μοναδικός, συνεχόμενος αριθμός Ζ που ΠΟΤΕ δεν
  // ξαναχρησιμοποιείται/αλλάζει). Βλ. app/api/z-report/route.js.
  zReports: [], // { id, number, mode, period, net, vat, total, byVatRate, byPaymentMethod, count, closedAt, closedBy, auto, printed }
