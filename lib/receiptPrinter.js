...
async function sendToPrinter(html, settings, extraJobs = []) {
  ...
  const attempt = async () => {
    ...
    // Οι raw εντολές (π.χ. άνοιγμα συρταριού) πρέπει να σταλούν σε ΞΕΧΩΡΙΣΤΟ print() με δικό
    // τους απλό config — αν μπουν στο ΙΔΙΟ print() με το pixel/html config (μέγεθος σελίδας σε
    // mm), το QZ Tray μπερδεύεται και προσπαθεί να αναλύσει το ίδιο το HTML σαν raw εντολή
    // ("Cannot parse ... into a raw HTML command").
    if (extraJobs.length > 0) {
      const rawConfig = qz.configs.create(printerName);
      await qz.print(rawConfig, extraJobs);
    }
