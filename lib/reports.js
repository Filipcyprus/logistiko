...
import { expenseOutstanding, purchaseTotal } from "@/lib/payables";
...
export function apAging(db, asOf) {
  ...
  const add = (key, name, number, date, outstanding) => {
    if (!rows.has(key)) rows.set(key, { name, ...emptyBuckets(), items: [] });
    const r = rows.get(key);
    ...
    r[b] = round2(r[b] + outstanding);
    r.total = round2(r.total + outstanding);
    r.items.push({ number, date, days, amount: outstanding });
  };
  ...
  for (const po of db.purchases || []) {
    ...
    add(po.supplierId || name, name, po.number, date, outstanding);
  }

  // Τιμολόγια εξόδων που ήρθαν αλλά δεν πληρώθηκαν (ενοίκιο, ρεύμα, …) — χρωστάμε εξίσου.
  for (const ex of db.expenses || []) {
    const outstanding = expenseOutstanding(db, ex);
    if (outstanding <= 0.005) continue;
    const date = (ex.date || "").slice(0, 10);
    if (!date || date > asOf) continue;

    const name = ex.supplier || "—";
    add(ex.supplierId || name, name, ex.number, date, outstanding);
  }
