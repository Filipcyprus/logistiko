      ...
      <div className="overflow-x-auto">
        <table className={`w-full ${pricing ? "min-w-[860px]" : "min-w-[520px]"}`}>
          ...
          <tbody className="divide-y divide-slate-100">
            {items.map((it, idx) => {
              const lt = pricing ? computeTotals([it]).total : null;
              return (
                <tr key={idx}>
                  ...
                  <td className="table-td">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const hasDiscount = pricing && p && (["cosmetic", "consumable"].includes(p.productType) || (p.customDiscountTiers || []).length > 0);
                  ...
                  {pricing && <td className="table-td text-right font-semibold whitespace-nowrap">{money(lt, currency)}</td>}
