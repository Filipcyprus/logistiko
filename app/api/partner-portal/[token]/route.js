...
export async function GET(request, { params }) {
  ...
  const jobs = (db.jobs || [])
    ...
    .map((j) => ({
      id: j.id, number: j.number, title: j.title, stageId: j.stageId,
      priority: j.priority, dueDate: j.dueDate, items: j.items || [], designs: j.designs || [],
      notes: j.notes, status: j.status, customerName: j.customerName,
      messages: j.messages || [], createdAt: j.createdAt, completedAt: j.completedAt,
    }));
